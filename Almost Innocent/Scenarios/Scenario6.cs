...
{
    public class Scenario6 : BaseScenario, IScenario
    {
        private static readonly Regex RegexQuestion = new("^(#|V|VICTIME |BLEU |P|PREUVE |VERT |L|LIEU |ORANGE |CR|CRIME |JAUNE |CO|COUPABLE |NOIR ){1}[A-F1-6]{1}$");
        private static readonly List<CardType> ScenarioCardTypes = [CardType.Guilty, CardType.Crime, CardType.Victim, CardType.Place, CardType.Evidence];

        private Scenario6(bool isAIEnabled, bool isGameStartWithClues, int totalSurveyTokens, int numberSurveyTokens, int cardSurveyTokens, int almostInnocentTokens)
            : base(new ScenarioBoard6(), ScenarioCardTypes, RegexQuestion, null, isAIEnabled, isGameStartWithClues, totalSurveyTokens, numberSurveyTokens, cardSurveyTokens, almostInnocentTokens)
        {
        ...
        private GuiltyCard GuiltyCard_AI { get; }
        ...
        public static Scenario6 Setup()
        {
            Console.WriteLine("----------------------------------------");
            Console.WriteLine($"\tScénario 6 - Bienvenue au tétard têtu !");
            Console.WriteLine("----------------------------------------");
            Console.WriteLine();
        ...
        protected override string Question(string question)
            => ScenarioBoard.Question(question, [CrimeCard_AI, VictimCard_AI, EvidenceCard_AI, PlaceCard_AI, GuiltyCard_AI]);
        ...
        private static Scenario6 Create(DIFFICULTY_LEVEL level, bool isAIEnabled, bool isGameStartWithClues)
            => level switch
            {
                DIFFICULTY_LEVEL.DETECTIVE => new Scenario6(isAIEnabled, isGameStartWithClues, 9, 7, 2, 0),
                DIFFICULTY_LEVEL.MEDIUM => new Scenario6(isAIEnabled, isGameStartWithClues, 10, 8, 2, 1),
                _ => new Scenario6(isAIEnabled, isGameStartWithClues, 12, 9, 3, 2),
            };
    }
}
